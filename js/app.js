...
provided by the Object prototype. Currently, they get a long list of properties,
but what they’d really like to know is how far the distance is from a
post (x, y) to the ranch (0,0).

Your job is to override the valueOf() method using the Fencepost prototype to
return the exact distance that a post is from the ranch. Here is the standard

algebraic distance formula:

square root of [ (x1 - x2)^2  +  (y1- y2)^2 ]
This formula includes points (x1, y1) for the post, and (x2, y2) for the ranch.
Since the ranch’s coordinates are always (0,0), you should be able to shorten
this procedure considerably. In other words, you shouldn’t need any subtraction
at all because of the zero values for the ranch coordinates.

In order to accomplish this, you can use the sqrt() method of JavaScript’s
built-in Math object! You can place any numerical expression as the parameter
for the Math.sqrt() method, and it will return the square root after evaluating
the expression. Use this method carefully in your override.
...
Begin by creating a function to override the toString method on the Fencepost
prototype. The function won’t need to take any parameters.
Inside the function, initialize a list variable to an empty string. We can use
list to keep track of connected posts.
Loop through the connectionsTo array with a for loop
(e.g., for (var i = 0; i < this.connectionsTo.length; i++))
Inside your for loop, concatenate the postNum property of each item in the
connectionsTo array on to the list string.
Lastly, return a string message in the format above with the correct output:
The postNum of the current post (e.g., Fence post #10)
The message Connected to posts: followed by each postNum value from the items
in the connectionsTo array, one per line
The output of calling valueOf() for the current post
(e.g., Distance from ranch: 5 yards)
The constructor and all of your previous prototype additions are provided for
you as a reference for property names and procedures.
